...
if fighter1_name and fighter2_name and fighter1_name == fighter2_name:
    ...
    with st.expander("Historical Fight Data"):
        ...
        with col1:
            ...
            if not history1.empty:
                # Improve visibility with adjusted column widths and no horizontal scroll
                st.dataframe(history1.style.set_properties(**{'text-align': 'left', 'white-space': 'pre-wrap', 'max-width': '150px'}).set_table_styles([
                    {'selector': 'th', 'props': [('max-width', '150px'), ('text-align', 'left')]},
                    {'selector': 'td', 'props': [('max-width', '150px'), ('text-align', 'left'), ('word-wrap', 'break-word')]}
        ...
        with col2:
            ...
            if not history2.empty:
                # Improve visibility with adjusted column widths and no horizontal scroll
                st.dataframe(history2.style.set_properties(**{'text-align': 'left', 'white-space': 'pre-wrap', 'max-width': '150px'}).set_table_styles([
                    {'selector': 'th', 'props': [('max-width', '150px'), ('text-align', 'left')]},
                    {'selector': 'td', 'props': [('max-width', '150px'), ('text-align', 'left'), ('word-wrap', 'break-word')]}
                ]), use_container_width=True)
            else:
    ...
    # Visual Comparison (Enhanced Fighter Profile Radar Chart)
    with st.expander("Visual Comparison"):
        ...
        fig_radar.update_layout(
            ...
            showlegend=True,
            title=dict(text="Fighter Profile (Normalized Stats)", font=dict(size=18)),
            height=600,
            width=800,
    ...
    st.write("Made by Shreyash Kumar")
else:
